...
fimovg version {version}

Copyright (C) 2019 Manuel Tognon <manu.tognon@gmail.com> <manuel.tognon@studenti.univr.it>

fimovg scores sequences likely to be transcription binding sites in ChIP-seq peaks

Usage:
    fimovg --cores [NCORES] --linear_genome [LINEAR_GENOME.fa] --vcf [VCF.vcf.gz] --bedfile [BEDFILE.bed] --motif [MOTIF.jaspar] [options]
    
    fimovg --cores [NCORES] --graph_genome [GRAPH_GENOME.xg] --bedfile [BEDFILE.bed] --motif [MOTIF.jaspar] [options]
...
(--o option or defalut value 'fimovg_out').
...
Run 'fimovg --help'to see all command-line options.
...
from fimovg import __version__, with_vg_pipeline, without_vg_pipeline
...
class FimoVGArgumentParser(ArgumentParser):
    """
    ...
    class FimoVGHelpFormatter(HelpFormatter):
    ...
    def __init__(self, *args, **kwargs):
        kwargs['formatter_class']=self.FimoVGHelpFormatter
        kwargs['usage']=kwargs['usage'].replace("{version}", __version__)
        super().__init__(*args, **kwargs)
        
    def error(self, msg):
        sys.stderr.write("Run 'fimovg --help' to see the usage\n")
...
def get_AP():
    """
        ...
        ----
        Returns:
            parser(FimoVGArgumentParser)
    
    """
    
    parser=FimoVGArgumentParser(usage=__doc__, add_help=False)
    ...
    group.add_argument("--o", type=str, help='name of the directory where the results will be stored [optional]',
                           nargs='?', const='fimoVG_out', default='fimoVG_out', metavar='OUTDIR')
